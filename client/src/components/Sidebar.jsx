...
import { Eraser, FileText, Hash, House, Image, SquarePen, Users, Scissors, LogOut} from 'lucide-react';
...
const Sidebar = ({sidebar, setsidebar}) => {
    const {user} = useUser();
    // const user = useUser();  // ❌ Wrong: This returns { user, isLoaded, isSignedIn }
    ...
    return (
        <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
            <div className='my-7 w-full'>
                <img src={user.imageUrl} alt="user" className='w-12 h-12 rounded-full mx-auto'/>
